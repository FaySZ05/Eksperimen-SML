...
def preprocess_data(df_raw):
    """
    Fungsi untuk melakukan preprocessing otomatis pada DataFrame.

    Langkah-langkah yang dilakukan:
    1. Mengubah kolom 'Timestamp' menjadi datetime dan menjadikannya indeks.
    2. Melakukan standarisasi fitur numerik ('Open', 'High', 'Low', 'Close', 'Volume').
    3. Menangani outlier menggunakan metode capping berdasarkan Z-score.

    Args:
        df_raw (pd.DataFrame): DataFrame mentah yang akan diproses.
    ...
    # 1. Menangani Timestamp
    if 'Timestamp' in df_processed.columns:
        df_processed['Timestamp'] = pd.to_datetime(df_processed['Timestamp'], unit='s')
        df_processed = df_processed.set_index('Timestamp')
    elif df_processed.index.name != 'Timestamp':
        if pd.api.types.is_numeric_dtype(df_processed.index):
            df_processed.index = pd.to_datetime(df_processed.index, unit='s')
        df_processed.index.name = 'Timestamp'
    ...
    existing_features_to_scale = [f for f in features_to_scale if f in df_processed.columns]

    if existing_features_to_scale:
        df_processed[existing_features_to_scale] = scaler.fit_transform(df_processed[existing_features_to_scale])
        print("\nDataFrame setelah standarisasi fitur:")
        print(df_processed.head())
        print("\nStatistik deskriptif fitur setelah standarisasi:")
        print(df_processed[existing_features_to_scale].describe())
    else:
        print("Tidak ada fitur yang relevan untuk distandarisasi ditemukan dalam DataFrame.")

    # 3. Penanganan Outlier menggunakan Capping (Z-score)
    print("\nMenangani Outlier menggunakan Capping (Z-score):")
    z_score_threshold = 3

    for feature in existing_features_to_scale:
...
# Pastikan `df` (DataFrame mentah Anda dari tahap pemuatan data) sudah tersedia.
# Jika `df` belum ada, Anda mungkin perlu memuat ulang data atau menjalankan sel sebelumnya.

# Memuat ulang data dari file CSV mentah untuk memastikan fungsi bekerja pada data asli
# Karena df di notebook sudah dimodifikasi oleh proses sebelumnya
files_in_dataset = os.listdir(path)
csv_files = [f for f in files_in_dataset if f.endswith('.csv')]

if csv_files:
    data_file_path = os.path.join(path, csv_files[0])
    df_raw_for_function = pd.read_csv(data_file_path)
    print("\n--- Menggunakan fungsi preprocess_data ---")
    df_cleaned = preprocess_data(df_raw_for_function)
    print("\nPreprocessing selesai. DataFrame siap digunakan:")
    print(df_cleaned.head())
    print(df_cleaned.info())
else:
    print("Tidak ada file CSV yang ditemukan untuk dimuat ulang.")
